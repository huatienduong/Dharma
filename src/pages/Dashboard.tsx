...
function SearchRow({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  return <div className="flex w-full items-center justify-center" role="search"><div className="flex w-full max-w-2xl items-center gap-2.5"><div className={cn("flex h-12 min-w-0 flex-1 items-center gap-2.5 rounded-full border border-border/70 bg-muted/80 px-3.5 shadow-sm", (listening || hasText) && "border-destructive/30 ring-2 ring-destructive/10")} onClick={() => inputRef.current?.focus()}><input ref={inputRef} value={value} onChange={(e) => onChange(e.target.value)} onKeyDown={(e) => { if (e.key === "Escape") onChange(""); }} placeholder="Tìm trên Dharma" aria-label="Tìm video" className="h-full min-w-0 flex-1 bg-transparent text-[15px] outline-none placeholder:text-muted-foreground" />{hasText && <button type="button" onClick={() => onChange("")} aria-label="Xóa từ khóa" className="flex h-7 w-7 items-center justify-center rounded-full text-muted-foreground hover:bg-accent"><X className="h-4 w-4" /></button>}<SearchIcon className="h-4 w-4 shrink-0 text-muted-foreground" /></div>{supported ? <button type="button" onClick={() => listening ? stop() : start(onChange)} aria-label="Tìm bằng giọng nói" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-border/70 bg-background shadow-sm hover:bg-accent"><Mic className="h-5 w-5" /></button> : <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-border/70 bg-background text-muted-foreground/50"><Loader2 className="h-5 w-5" /></span>}</div></div>;
}
